...
void send_data_message_wc() //with COP
{
  ...
  data_to_send[6] = left_leg->AnkleMoment_Mod;//left_leg->state;
  data_to_send[7] = (left_leg->sign * left_leg->PID_Setpoint);

  if (FLAG_TOE_HEEL_SENSORS) {  // TN 5/8/19
    data_to_send[8] = (left_leg->FSR_Toe_Average);//((left_leg->fsr_percent_thresh_Toe + left_leg->fsr_percent_thresh_Heel) * left_leg->fsr_Combined_peak_ref);
    data_to_send[9] = (left_leg->FSR_Heel_Average);//(left_leg->FSR_Combined_Average);
  } else if (FLAG_BALANCE) {
