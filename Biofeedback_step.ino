...
void biofeedback_step_baseline(Leg* leg) {
  if (leg->HS1 == 0 || leg->HS4 == 0) {
    ...
    leg->stridelength_baseline = leg->stridetime_baseline * 100; //unit:cm
...
void biofeedback_step_update (Leg* leg) {
  if (leg->HS1 == 0 || leg->HS2 == 0) {
    ...
    leg->stridelength_update = leg->stridetime_update * 100; //unit:cm
    ...
    if (leg->stridelength_target != 0 && leg->stridelength_update >= leg->stridelength_target) {
      ...
      //leg->stridelength_update_scale = leg->stridelength_update / leg->stridelength_target;
