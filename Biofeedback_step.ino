void takestridetime(Leg* leg) { //take the duration of L/R stride
  ...
  if (leg_Heel_Strike_Count=1) {
    leg->HS1=millis();
  } else if (leg_Heel_Strike_Count=4) {
    leg->HS4=millis();
    leg->stridetime=(leg->HS4-leg->HS1)/3; //calculate time (ms) to complete 3 strides
  } else if (leg_Heel_Strike_Count>4) {
    leg->Heel_Stride_Count=0;
  }
  return;
}
...
void biofeedback_step_baseline(Leg* leg) {
  ...
  leg->stridelength_baseline=leg->treadmill_speed*leg->stridetime/1000;//unit:m
...
void biofeedback_step_update (Leg* leg) {
  ...
  leg->stridelength_update=leg->treadmill_speed*leg->stridetime/1000;//unit:m
