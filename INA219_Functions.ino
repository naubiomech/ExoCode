...
int readBatteryVoltage() { // Read the battery voltage from the INA219 over I2C
  Wire.beginTransmission(INA219_ADR); // Start a new transmission
  Wire.write(INA219_BUS);             // Specify the register we want to read (bus/battery voltage)
  Wire.endTransmission();             // End transmission
  Wire.requestFrom(INA219_ADR, 2);    // Request two bytes of data from the INA219
  int BatVolt = Wire.read();          // Get the first byte of the battery voltage
  BatVolt = BatVolt << 8;                // Shift the data over by 8 bits since the voltage is 16 bits total
  int BatVolt2 = Wire.read();         // Get the second byte
...
int readShuntVoltage() { // Read the shunt resistor voltage from the INA219 over I2C
  Wire.beginTransmission(INA219_ADR);   // Start a new transmission
  Wire.write(INA219_SHUNT);             // Specify the register we want to read (shunt voltage)
  Wire.endTransmission();               // End transmission
  Wire.requestFrom(INA219_ADR, 2);      // Request two bytes of data from the INA219
  int ShuntVolt = Wire.read();          // Get the first byte of the shunt voltage
  ShuntVolt = ShuntVolt << 8;              // Shift the data over by 8 bits since the voltage is 16 bits total
  int ShuntVolt2 = Wire.read();         // Get the second byte
