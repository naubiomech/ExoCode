 ...
 *  - Add (long) assignments and casts to TimerOne_G::read() to ensure calculations involving tmp, ICR1 and TCNT1 aren't truncated
 ...
 *
 *  See Google Code project http://code.google.com/p/arduino-TimerOne_G/ for latest
 */
#ifndef TimerOne_G_cpp
#define TimerOne_G_cpp

#include "TimerOne_G_G.h"

TimerOne_G Timer1;              // preinstatiate
...
void TimerOne_G::initialize(long microseconds)
{
...
void TimerOne_G::setPeriod(long microseconds)		// AR modified for atomic access
{
...
void TimerOne_G::setPwmDuty(char pin, int duty)
{
...
void TimerOne_G::pwm(char pin, int duty, long microseconds)  // expects duty cycle to be 10 bit (1024)
{
...
void TimerOne_G::disablePwm(char pin)
{
  if(pin == 1 || pin == 9)       TCCR1A &= ~_BV(COM1A1);   // clear the bit that enables pwm on PB1
  else if(pin == 2 || pin == 10) TCCR1A &= ~_BV(COM1B1);   // clear the bit that enables pwm on PB2
}

void TimerOne_G::attachInterrupt(void (*isr)(), long microseconds)
{
  if(microseconds > 0) setPeriod(microseconds);
...
void TimerOne_G::detachInterrupt()
{
  TIMSK1 &= ~_BV(TOIE1);                                   // clears the timer overflow interrupt enable bit 
															// timer continues to count without calling the isr
}

void TimerOne_G::resume()				// AR suggested
{ 
  TCCR1B |= clockSelectBits;
}

void TimerOne_G::restart()		// Depricated - Public interface to start at zero - Lex 10/9/2011
{
	start();				
}

void TimerOne_G::start()	// AR addition, renamed by Lex to reflect it's actual role
{
  unsigned int tcnt1;
...
void TimerOne_G::stop()
{
  TCCR1B &= ~(_BV(CS10) | _BV(CS11) | _BV(CS12));          // clears all clock selects bits
}

unsigned long TimerOne_G::read()		//returns the value of the timer in microseconds
