...
TEST(transmission_base, process_one_send){
	...
	float fake_data[1] = {0};
	transmission->copyToSend((float*)fake_data);
...
TEST(transmission_base, process_ten_send){
	MockTransceiver* transceiver = new MockTransceiver();
...
TEST(transmission_base, process_one_both){
	...
	EXPECT_CALL(*transmission, processData(NULL,NULL))
		.Times(1)
		.WillOnce(InvokeWithoutArgs(transmission, &MockBasicTransmission::copyReceiveToSend));
...
TEST(transmission_base, process_ten_both){
	MockTransceiver* transceiver = new MockTransceiver();
