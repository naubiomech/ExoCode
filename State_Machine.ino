...
void State_Machine_Toe_Heel_Sensors(Leg * leg) {  // TN 5/8/19
  switch (leg->state)
  {
    case 1: //Swing
      ...
      else if (leg->p_steps->curr_voltage_Toe > leg->fsr_percent_thresh_Toe * leg->fsr_Toe_peak_ref) //&& (leg->p_steps->curr_voltage > leg->fsr_percent_thresh_Heel * leg->fsr_Combined_peak_ref))
      {
        leg->state_count_13++;
    ...
    case 2: //Early Stance //GO 5/19/19
      if ((leg->set_2_zero == 1) && (leg->One_time_set_2_zero)) {
        ...
        if (leg->state_count_21 >= state_counter_th) {
          ...
          if (leg->Previous_Dorsi_Setpoint_Knee <= leg->Dorsi_Setpoint_Knee) { //Increment Dorsi Setpoint for Bang-Bang & Proportional GO - 5/19/19
