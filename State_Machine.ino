...
void State_Machine_Toe_Heel_Sensors(Leg * leg) {  // TN 5/8/19
  switch (leg->state)
  {
    case 1: //Swing
      ...
      else if (leg->FSR_Toe_Average > leg->fsr_percent_thresh_Toe * leg->fsr_Toe_peak_ref) //&& (leg->p_steps->curr_voltage > leg->fsr_percent_thresh_Heel * leg->fsr_Combined_peak_ref))
      {
        leg->state_count_13++;
        ...
        if (leg->state_count_13 >= state_counter_th)
        {
          if (Control_Mode == 100) { //Increment set point for bang-bang GO - 5/19/19
            ...
            if (abs(leg->Previous_Setpoint_Knee_Pctrl) <= abs(leg->Setpoint_Knee)) {
              leg->p_steps->Setpoint_Knee = leg->Previous_Setpoint_Knee_Pctrl + (leg->Setpoint_Knee - leg->Previous_Setpoint_Knee_Pctrl) * leg->coef_in_3_steps; //Increment when the new setpoint is higher than the old
            } else {
              leg->p_steps->Setpoint_Knee = leg->Previous_Setpoint_Knee_Pctrl - (leg->Previous_Setpoint_Knee_Pctrl - leg->Setpoint_Knee) * leg->coef_in_3_steps; //Decrement when the new setpoint is lower than the old
            }
            if (leg->p_steps->Setpoint_Knee == 0) {
      ...
      else if ((leg->FSR_Heel_Average > leg->fsr_percent_thresh_Heel * leg->fsr_Heel_peak_ref && leg->FSR_Toe_Average < leg->fsr_percent_thresh_Toe * leg->fsr_Toe_peak_ref)) { //Heel active but not toe
        leg->state_count_12++;
    ...
    case 3: //Late Stance
      ...
      if (leg->FSR_Toe_Average < leg->fsr_percent_thresh_Toe * leg->fsr_Toe_peak_ref) // TN 5/21/19
      {
        leg->state_count_31++;
  ...
  if (Control_Mode == 2 || Control_Mode == 3 || Control_Mode == 4) {  // TN 5/20/19 set the knee control in state 2 and 3
    //if (leg->state == 3) {
      ...
      leg->PID_Setpoint_Knee = leg->Setpoint_Knee_Pctrl ;
    //}
