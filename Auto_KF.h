...
void Auto_KF(Leg* leg, int Control_Mode) {
  ...
  {
    if (abs(leg->Setpoint_Ankle * leg->coef_in_3_steps_Ankle) != abs(leg->Max_Measured_Torque)) { //If target is greater than 99% of setpoint
      leg->auto_KF_update = true;                       // Raise auto KF flag
    }
  } else if (leg->state == 3 && (Control_Mode == 3 || Control_Mode == 4)) //If proportional
  {
  ...
  if (leg->state == 1 && leg->auto_KF_update) {
    ...
    if (Control_Mode == 100) { //If bang-bang
      ...
      if (leg->Max_Measured_Torque * leg->Setpoint_Ankle * leg->coef_in_3_steps_Ankle <= 0) {
        // if the sign are not concord, no auto update of KF
        leg->auto_KF_update = false; //added after test of 11/7/18
        return;
      }

      if ( abs((leg->Max_Measured_Torque - (leg->Setpoint_Ankle * leg->coef_in_3_steps_Ankle)) / (leg->Setpoint_Ankle * leg->coef_in_3_steps_Ankle)) < 0.01) {
        leg->auto_KF_update = false; //added after test of 11/7/18
        return;
      }// if the error is less than 1% no need to change KF

      if (leg->coef_in_3_steps_Ankle > 0) {
        leg->ERR = leg-> ERR + (leg->Max_Measured_Torque - (leg->Setpoint_Ankle * leg->coef_in_3_steps_Ankle)) / (leg->Setpoint_Ankle * leg->coef_in_3_steps_Ankle); //Calculate a running sum of the relative error
        leg->KF = leg->KF - ((leg->Max_Measured_Torque - (leg->Setpoint_Ankle * leg->coef_in_3_steps_Ankle)) / (leg->Setpoint_Ankle * leg->coef_in_3_steps_Ankle) * 0.6 + leg->ERR * 0.01); //changed from 0.4 to 0.6 after test of 11/7/18
      }
    } else if ((Control_Mode == 3) || (Control_Mode == 4)) { //If proportional
